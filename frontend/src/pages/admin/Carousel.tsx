...
export default function AdminCarousel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await api.put(`/carousel/${editingItem._id}`, carouselData);
        toast({ title: 'Success', description: 'Carousel item updated successfully' });
      } else {
        await api.post('/carousel', carouselData);
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-3xl font-bold text-foreground">Carousel</h1>
            <p className="text-muted-foreground mt-1">Manage homepage carousel slides</p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={(open) => {
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingItem ? 'Edit Carousel Item' : 'Add New Carousel Item'}</DialogTitle>
                <DialogDescription>
                  Fill in the carousel item details below
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Optional link when users click "Shop Now" button
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Only active items will be displayed on the homepage
                    </p>
                  </div>
                  <Switch
                    id="isActive"
                ...
                <DialogFooter>
                  ...
                  <Button type="submit" disabled={saving}>
                    ...
                    {editingItem ? 'Update Carousel' : 'Create Carousel'}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
                  ...
                  {carouselItems.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                        No carousel items found. Create your first carousel slide!
                      </TableCell>
                    </TableRow>
                  ) : (
